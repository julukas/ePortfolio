...
class TreasureMaze(object):
    ...
    def __init__(self, maze, pirate=(0,0), priority_cell=(0, 0)):
        self._maze = np.array(maze)
        nrows, ncols = self._maze.shape
        self.target = (nrows-1, ncols-1)   # target cell where the "treasure" is
        self.free_cells = [(r,c) for r in range(nrows) for c in range(ncols) if self._maze[r,c] == 1.0]
        self.free_cells.remove(self.target)
        self.priority_cell = priority_cell
        self.priority_visited = False
    ...
    def get_reward(self):
        ...
        nrows, ncols = self.maze.shape
        if (pirate_row, pirate_col) == self.priority_cell and not self.priority_visited:
            self.priority_visited = True
            return 1.0
    ...
    def draw_env(self):
        ...
        for r in range(nrows):
            for c in range(ncols):
                if canvas[r,c] > 0.0:
                    canvas[r,c] = 1.0
        r, c = self.priority_cell
        canvas[r, c] = 0.6 if (r, c) in self.visited else 0.3
        # draw the pirate
    ...
    def game_status(self):
        ...
        # Require the priority cell to be visited before winning
        if (pirate_row == nrows - 1 and pirate_col == ncols - 1):
            if self.priority_visited:
                return 'win'
            else:
                return 'not_over'  # Treasure reached but priority cell not visited
